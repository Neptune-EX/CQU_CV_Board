...
% 项目参考overleaf武汉大学简历模板：https://www.overleaf.com/latex/templates/whuwu-han-da-xue-zhong-wen-jian-li-mo-ban/dbkvxrqjmzpd
...
\titleformat{\section}					    % 将原标题前面的数字取消了
  ...
  {}{0em}                      			  % 可用于添加全局标题前缀
  {}                           			  % 可用于添加代码
  [{\color{CQU_Blue}\titlerule}]            % 标题下方加一条线
...
% 页面大小与页边距，按需求调整
\usepackage[top=2cm,     % 上边距（为页眉留出空间）
			bottom=1.5cm,  % 下边距（为页脚留出空间）
			left=1.2cm, 
...
% 自定义主题色
% 重大蓝（来自官方配色方案，参考https://www2.cqu.edu.cn/Channel/000-002-001-003/1/index.html）
\definecolor{CQU_Blue}{RGB}{2, 82, 159}
